...
step :: Int -> [State] -> Either [State] State
step num = foldl step' (Left [])
  where
    step' winning@(Right _) _ = winning
    step' (Left acc) state = 
      let updatedState = mark num state
      in if hasWon updatedState
        then Right updatedState
        else (Left (updatedState : acc))

play :: [Int] -> [State] -> Int
play (num : nums) states =
  case step num states of
    Left updatedStates -> play nums updatedStates
    Right winningState -> computeScore winningState
  where
